...
void Http::handleRoot()
{
    ...
    snprintf_P(html, sizeof(html),
               PSTR(
                   ...
                   "<tr><td>电压</td><td><span id='busvoltage'>%.2f</span> V</td></tr>"
                   "<tr><td>电流</td><td><span id='current_mA'>%.2f</span> mA</td></tr>"
                   "<tr><td>功率</td><td><span id='busPower'>%.2f</span> W</td></tr>"
                   "</tbody></table>"
                   "</div>"),
               (mode == WIFI_STA ? PSTR("STA") : (mode == WIFI_AP ? PSTR("AP") : PSTR("AP STA"))),
               WiFi.SSID().c_str(), WiFi.RSSI(),
               WifiMgr::networkAddress().toString().c_str(), (globalConfig.wifi.is_static ? PSTR("静态IP") : PSTR("DHCP")),busvoltage,current_mA,busPower);
    server->sendContent_P(html);
    // TAB 1 End
...
void Http::handleGetStatus()
{
    ...
    snprintf_P(html, sizeof(html), PSTR(",\"busvoltage\":%.2f,\"busPower\":%.2f,\"current_mA\":%.2f"), busvoltage,busPower,current_mA);
    server->sendContent_P(html);
